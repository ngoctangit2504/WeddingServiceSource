...
import com.wedding.backend.repository.payment.*;
...
import com.wedding.backend.util.extensions.ConvertObjectToJsonExtension;
    ...
    public ResponseEntity<?> vnpayReturnView(ViewPaymentReturnDto response) {
        ...
        try {
            if (Objects.equals(response.getPaymentStatus(), "00")) {
                ...
                responseEntity = new ResponseEntity<>(new BaseResult(false, MessageUtil.MSG_PROCESS_TRANSACTION_PAYMENT_FAIL), HttpStatus.BAD_REQUEST);
            }
        } catch (Exception ex) {
